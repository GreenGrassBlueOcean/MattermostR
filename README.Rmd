...
MattermostR is an R package for interacting with the [Mattermost REST API v4](https://api.mattermost.com/).
It covers messaging, file management, channel and team administration, reactions, bots, slash commands, and more.
...
Credentials are resolved from environment variables first, then R options.
...
By default, HTTP errors raise a `mattermost_error` condition (stops execution).
Switch to the legacy silent mode if you prefer `NULL` returns:
